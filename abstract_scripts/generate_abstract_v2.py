...
ALL_SHAPES = ["circle", "square", "triangle", "diamond", "pentagon",
              "hexagon", "star", "cross", "arrow", "octagon"]

# Shapes grouped by number of sides (for "sides increases" patterns)
SIDES_ORDER = ["triangle", "square", "pentagon", "hexagon", "octagon"]

# ─── EASY generators (genuine but approachable) ───────────────────────────────

def gen_sides_increase(qid):
    """Easy: number of sides increases by 1 each step -- triangle, square, pentagon, hexagon"""
    color = random.choice(COLORS)
    start_idx = random.randint(0, 1)  # start at triangle or square
    seq_shapes = SIDES_ORDER[start_idx:start_idx+4]
    correct_shape = SIDES_ORDER[start_idx+4] if start_idx+4 < len(SIDES_ORDER) else SIDES_ORDER[start_idx+3]
    r = 28
    ...
    wrong_opts = [
        seq_shapes[-1],   # repeat last
        seq_shapes[-2],   # two back
        SIDES_ORDER[(start_idx+2) % len(SIDES_ORDER)],
        "circle",
    ]
    ...
    return {
        ...
        "explanation": f"Each shape has one more side than the previous. The sequence adds a side each step."
    }

def gen_rotation_with_fill(qid):
    """Easy: shape rotates 90 degrees AND alternates fill simultaneously."""
    shape = random.choice(["triangle", "arrow", "diamond", "pentagon", "star"])
    color = random.choice(COLORS)
    ...
    correct = (0, color)  # 360=0 and filled
    wrong_opts = [
        (0, "white"),    # right rotation wrong fill
        (270, color),    # previous rotation right fill
    ...
    return {
        ...
        "sequence_images": seq_files, "option_images": opt_files,
        "correct_answer": correct_label,
        "explanation": f"Two rules: the shape rotates 90 degrees AND alternates filled/empty. Both must be correct."
    }
...
def gen_size_increase_series(qid):
    """Easy: shape grows larger each step with consistent color."""
    ...
    for i, s in enumerate(sizes):
        ...
        def draw(dwg, size, s=s):
            draw_shape_element(dwg, shape, 50, 50, s, color)
        make_svg(fname, draw)
        seq_files.append(fname)

    wrong_sizes = [34, 26, 46, 38]
    all_sizes, correct_label = shuffle_with_correct(correct_size, wrong_sizes)
...
def gen_shape_mirror(qid):
    """Easy: shape alternates between normal and mirrored (180 degree rotation)."""
    shape = random.choice(["triangle", "arrow", "star", "pentagon"])
    color = random.choice(COLORS)
    ...
    return {
        ...
        "sequence_images": seq_files, "option_images": opt_files,
        "correct_answer": correct_label,
        "explanation": "The shape alternates between upright and flipped (180 degrees). After flipped, the next is upright."
    }

def gen_odd_one_out_symmetry(qid):
    """Easy: four shapes are symmetric, one is rotated to break symmetry."""
    shape = random.choice(["triangle", "arrow", "pentagon", "star"])
    color = random.choice(COLORS)
    majority_rot = 0
    outlier_rot = random.choice([45, 90, 135])
    ...
    return {
        ...
        "correct_answer": correct_label,
        "explanation": "Four figures point in the same direction. One is rotated differently and breaks the pattern."
    }
